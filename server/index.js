...
// Track how long price stays above/below PTB within a slot
...
// Load saved theme
if(localStorage.getItem('btcTheme')==='light'){document.body.classList.add('light-mode');document.addEventListener('DOMContentLoaded',()=>{const b=document.getElementById('themeBtn');if(b)b.textContent='🌙 Dark'})}
let btcWs=null,latestBtcPrice=null;
let polyWs=null,latestChainlinkPrice=null;
let lastChainlinkSlot=null;

// ═══ CHAINLINK CANDLE BUILDER ═══
...
// ═══ SIMPLE PTB: snapshot current price at each 5-minute mark ═══
...
async function loadPtb(){
  ...
  try{
    ...
    if(d&&d.slot===currentSlot&&d.price>50000&&d.price<200000){
      ...
      console.log('📌 PTB restored from local cache: $'+d.price.toFixed(2));
      return;
    }
  }catch(e){}
  ...
  try{
    ...
    if(d.price&&d.price>50000&&d.price<200000){
      ...
      console.log('📌 PTB restored from server: $'+d.price.toFixed(2));
    }
  }catch(e){}
}
loadPtb();
...
function startPtbTimer(){
  ...
  console.log('⏱️ Next PTB capture in '+(delay/1000).toFixed(1)+'s');
  ...
  setTimeout(()=>{
    ...
    if(ptbPrice&&markets[5]){
      ...
      savePtb(ptbPrice);
      console.log('🎯 PTB SET: $'+ptbPrice.toFixed(2)+' at '+new Date().toLocaleTimeString());
    }
    setInterval(()=>{
      ...
      if(p&&markets[5]){
        ...
        savePtb(p);
        console.log('🎯 PTB SET: $'+p.toFixed(2)+' at '+new Date().toLocaleTimeString());
      }
    },slotMs);
  },delay);
}
...
// ═══ POLYMARKET ODDS (the market's own prediction) ═══
...
// Fetch every 10 seconds — this is our most valuable signal
setInterval(fetchPolymarketOdds,10000);
setTimeout(fetchPolymarketOdds,3000);

// ═══ ORDERBOOK IMBALANCE — real-time bid/ask depth from Polymarket CLOB ═══
...
async function connectOrderbook(){
  try{
    const r=await fetch('/api/polymarket-odds');
    const data=await r.json();
    const tokens=data.clobTokenIds||[];
    if(tokens.length<2)return;
    
    // Don't reconnect if same tokens
    ...
    clobWs.onopen=function(){
      clobWs.send(JSON.stringify({type:'market',assets_ids:tokens}));
      orderbookData.connected=true;
    ...
    clobWs.onmessage=function(e){
      try{
        ...
        if(msg.event_type==='book'||msg.bids||msg.asks){
          ...
          const bids=msg.bids||[];
          const asks=msg.asks||[];
          const bidDepth=bids.reduce((sum,b)=>sum+parseFloat(b.size||0)*parseFloat(b.price||0),0);
          
          if(tokenId===tokens[0])orderbookData.upBidDepth=bidDepth;
          else if(tokenId===tokens[1])orderbookData.downBidDepth=bidDepth;
          
          const total=orderbookData.upBidDepth+orderbookData.downBidDepth;
          orderbookData.imbalance=total>0?(orderbookData.upBidDepth-orderbookData.downBidDepth)/total:0;
          orderbookData.lastUpdate=Date.now();
        }
      }catch(e){}
    };
    clobWs.onclose=function(){orderbookData.connected=false};
    clobWs.onerror=function(){orderbookData.connected=false};
  }catch(e){console.log('Orderbook error:',e)}
}
setTimeout(connectOrderbook,5000);
setInterval(connectOrderbook,60000); // Check every 60s for new market

// ═══ WHALE TRACKER: Follow Winner55555's trades ═══
...
async function fetchWhaleTrades(){
  try{
    ...
    if(data.whaleBet){
      ...
      console.log('🏆 Top trader: '+data.topTrader+' ('+(data.winRate*100).toFixed(0)+'% win rate) → '+data.whaleBet);
    }else if(data.topTrader){
      whaleBet.topTrader=data.topTrader;
      whaleBet.winRate=data.winRate||0;
      console.log('🏆 Top trader: '+data.topTrader+' ('+(data.winRate*100).toFixed(0)+'%) — no bet this slot yet');
    }
  }catch(e){}
}
...
// ★★★ STRATEGY 0B: POSITION STICKINESS (where has price BEEN this slot?) ★★★
...
// If price was below PTB 90% of the slot and just spiked $5 above, still likely NO
{
  ...
  if(positionHistory.length>=3){
    const w=8; // Very heavy weight — this is one of the most reliable signals
    ...
    if(abovePct>0.7){
      sc+=w*(abovePct-0.5)*3;
      sigs.push({name:'★ Position History',signal:'ABOVE '+(abovePct*100).toFixed(0)+'% of slot (avg +$'+avgDiff.toFixed(0)+') → YES BIAS',strength:w*abovePct});
    }else if(abovePct<0.3){
      sc-=w*(0.5-abovePct)*3;
      sigs.push({name:'★ Position History',signal:'BELOW '+(100-abovePct*100).toFixed(0)+'% of slot (avg $'+avgDiff.toFixed(0)+') → NO BIAS',strength:w*(1-abovePct)});
    }else{
      // Mixed — slight bias based on average position
    ...
    if(Math.abs(pb.trend)>5){
      ...
      sigs.push({name:'Slot Trend',signal:pb.trend>0?'DRIFTING UP ↑':'DRIFTING DOWN ↓',strength:trendW});
      tw+=trendW;
    }
  }
}
...
const b=bb(jp);
if(b){const w=1.5;
if(cur>b.mid){sc+=w;sigs.push({name:'Bollinger',signal:'ABOVE MID → BULLISH',strength:w})}
else{sc-=w;sigs.push({name:'Bollinger',signal:'BELOW MID → BEARISH',strength:w})}
tw+=w}
...
// 6. Williams %R — REMOVED (backtest: 26.4% accuracy — anti-predictive)
...
// 8. Mean Reversion — REMOVED (backtest: 21.9% accuracy — worst strategy)
...
// 10. OBV — REMOVED (adds noise on 5-min timeframes)
...
// 14. Fear & Greed — REMOVED (updates daily, useless for 5-min)

// 15. Funding Rate (momentum — contrarian loses on 5-min)
if(Math.abs(externalData.fundingRate)>0.01){const w=1;if(externalData.fundingRate>0.03){sc+=w;sigs.push({name:'Funding Rate',signal:'HIGH → BULLISH momentum',strength:w})}else if(externalData.fundingRate<-0.01){sc-=w;sigs.push({name:'Funding Rate',signal:'NEGATIVE → BEARISH momentum',strength:w})}tw+=w}
...
// 19. LONG/SHORT RATIO (momentum following — backtest shows contrarian loses)
if(longShortRatio>0){
  const w=1.5;
  if(longShortRatio>1.5){sc+=w;sigs.push({name:'Long/Short ('+longShortRatio.toFixed(2)+')',signal:'LONGS DOMINANT → BULLISH',strength:w})}
  else if(longShortRatio<0.7){sc-=w;sigs.push({name:'Long/Short ('+longShortRatio.toFixed(2)+')',signal:'SHORTS DOMINANT → BEARISH',strength:w})}
  tw+=w*0.5;
}
...
// 21. Volume Profile — REMOVED (unreliable with browser-limited data)

// ★★★ 21A. CHAINLINK CANDLE MOMENTUM (uses exact Polymarket price source) ★★★
// Analyzes the last 2-6 completed Chainlink 5-min candles
if(chainlinkCandles.length>=3){
  const w=5; // High weight — these are from the EXACT Polymarket price source
  ...
  if(greenPct>=0.67){
    ...
    sc+=w*strength;
    sigs.push({name:'★ CL Candles',signal:greenCount+'/'+recent.length+' GREEN → BULLISH (+$'+totalMove.toFixed(0)+')',strength:w*strength});
  }else if(greenPct<=0.33){
    const strength=(1-greenPct)*1.5;
    sc-=w*strength;
    sigs.push({name:'★ CL Candles',signal:(recent.length-greenCount)+'/'+recent.length+' RED → BEARISH ($'+totalMove.toFixed(0)+')',strength:w*strength});
  }else{
    sigs.push({name:'★ CL Candles',signal:'MIXED ('+greenCount+'/'+recent.length+')',strength:0});
  }
  tw+=w;
  
  // ★★★ 21B. LAST 2 CANDLE PATTERN (strongest recent signal) ★★★
  ...
  if(prev.close<prev.open && last.close>last.open){
    ...
    if(recoverySize>0.5){
      ...
      sigs.push({name:'★ CL Pattern',signal:'V-RECOVERY → STRONG BULLISH',strength:w2*Math.min(2,recoverySize)});
    }
  }
  // Dump pattern: green then red (inverted V) = strong bearish
    ...
    if(dumpSize>0.5){
      ...
      sigs.push({name:'★ CL Pattern',signal:'REJECTION → STRONG BEARISH',strength:w2*Math.min(2,dumpSize)});
    }
  }
  // Continuation: both green = momentum
  else if(prev.close>prev.open && last.close>last.open){
    sc+=w2;
    sigs.push({name:'★ CL Pattern',signal:'DOUBLE GREEN → BULLISH CONTINUATION',strength:w2});
  }
  // Continuation: both red = momentum
  else if(prev.close<prev.open && last.close<last.open){
    sc-=w2;
    sigs.push({name:'★ CL Pattern',signal:'DOUBLE RED → BEARISH CONTINUATION',strength:w2});
  }
  tw+=w2;
  
  // ★★★ 21C. CANDLE SIZE ANALYSIS ★★★
  ...
  if(lastSize>avgSize*2){
    // Last candle was unusually large — momentum likely continues
...
// ★★★ POLYMARKET ODDS (the market itself is the best predictor) ★★★
...
if(polymarketOdds.upPrice&&(Date.now()-polymarketOdds.lastFetch)<60000){
  const w=12; // HIGHEST weight — crowd intelligence with real money
  ...
  if(Math.abs(bias)>0.05){ // Use even slight odds (not exactly 50/50)
    ...
    sigs.push({name:'★★ Polymarket Odds',signal:'Up '+pct+'¢ / Down '+(100-pct)+'¢'+(upProb>0.7?' → STRONG YES':upProb<0.3?' → STRONG NO':''),strength:w*Math.abs(bias)});
    tw+=w;
  }
}
...
// ★★★ ADAPTIVE REWEIGHTING — apply learned multipliers to all signals ★★★
...
if(Object.keys(signalMultipliers).length>0&&sigs.length>3){
  ...
  if(learned.length>0){
    ...
    sigs.push({name:'★ Learning',signal:'Best: '+topSignal[0]+' ('+topSignal[1].mult.toFixed(1)+'x) | Worst: '+worstSignal[0]+' ('+worstSignal[1].mult.toFixed(1)+'x)',strength:0});
  }
}
...
const priceDiffPct=((price-ptb)/ptb*100);

// Track PTB position + velocity over time within this slot
const currentSlotKey=Math.floor(Date.now()/300000);
if(ptbTracker.lastSlot!==currentSlotKey){
  ptbTracker={aboveCount:0,belowCount:0,totalChecks:0,lastSlot:currentSlotKey,prices:[],firstPrice:null};
}
ptbTracker.totalChecks++;
if(priceAbovePtb)ptbTracker.aboveCount++;
else ptbTracker.belowCount++;
ptbTracker.prices.push(price);
if(!ptbTracker.firstPrice)ptbTracker.firstPrice=price;

// ═══ HYBRID: MEAN REVERSION + MOMENTUM + POLYMARKET ═══
//
// Mean reversion insight: big fast moves in first 60s tend to reverse
//   - Price spikes $80 above PTB → likely comes back → bet NO
//   - Price drops $80 below PTB → likely bounces → bet YES
//
// Momentum insight: steady drift in one direction tends to continue
//   - Price slowly climbing, consistently above PTB → bet YES
//
// Key: SPEED of move matters. Fast spike = reversion. Slow drift = momentum.
//
// Polymarket odds: if market disagrees with price direction, trust the market

const aboveRatio=ptbTracker.totalChecks>0?ptbTracker.aboveCount/ptbTracker.totalChecks:0.5;
const dominance=Math.max(aboveRatio,1-aboveRatio);

// CHART TREND: count up-ticks vs down-ticks in the stored prices
let upTicks=0,downTicks=0,trendSlope=0;
if(ptbTracker.prices.length>=3){
  for(let i=1;i<ptbTracker.prices.length;i++){
    if(ptbTracker.prices[i]>ptbTracker.prices[i-1])upTicks++;
    else if(ptbTracker.prices[i]<ptbTracker.prices[i-1])downTicks++;
  }
  // Slope: linear regression over all stored prices (positive = trending up)
  const n=ptbTracker.prices.length;
  let sumX=0,sumY=0,sumXY=0,sumX2=0;
  for(let i=0;i<n;i++){sumX+=i;sumY+=ptbTracker.prices[i];sumXY+=i*ptbTracker.prices[i];sumX2+=i*i}
  trendSlope=(n*sumXY-sumX*sumY)/(n*sumX2-sumX*sumX); // $/tick
}
const totalTicks=upTicks+downTicks;
const upRatio=totalTicks>0?upTicks/totalTicks:0.5;
const trendUp=upRatio>0.5;
const trendStrength=Math.abs(upRatio-0.5)*2; // 0=flat, 1=all one direction

// Calculate velocity: how fast did price move?
const lookback=Math.min(ptbTracker.prices.length,6);
const earlyPrice=ptbTracker.prices[Math.max(0,ptbTracker.prices.length-lookback)];
const recentMove=Math.abs(price-earlyPrice);
const totalMove=Math.abs(price-ptb);

const spikeRatio=totalMove>5?recentMove/totalMove:0;

const isBigGap=priceDiff>40;
const isSpike=spikeRatio>0.7;
const useReversion=isBigGap&&isSpike;

// RSI REVERSION BOOSTER — overbought/oversold confirms reversion
let rsiReversion=0; // -1 to +1, positive = confirms reversion (price should come back down if above)
if(rs){
  if(rs>75&&priceAbovePtb)rsiReversion=Math.min((rs-70)/30,1); // Overbought + above PTB → fade
  else if(rs<25&&!priceAbovePtb)rsiReversion=Math.min((25-rs)/25,1); // Oversold + below PTB → fade
}

// VOLUME SPIKE DETECTION — big volume in first 30s often means reversal
// Everyone piled in at once, no buyers/sellers left to push further
let volumeSpike=false;
let volumeSpikeRatio=0;
if(vols.length>=10){
  const recentVols=vols.slice(-6); // Last ~30s
  const olderVols=vols.slice(-20,-6); // Previous period
  if(olderVols.length>=5){
    const recentAvg=recentVols.reduce((a,b)=>a+b,0)/recentVols.length;
    const olderAvg=olderVols.reduce((a,b)=>a+b,0)/olderVols.length;
    volumeSpikeRatio=olderAvg>0?recentAvg/olderAvg:1;
    volumeSpike=volumeSpikeRatio>2.0; // Volume 2x+ higher than normal
  }
}

// POLYMARKET SIGNAL: does market agree or disagree with current price direction?
let polyAgrees=null; // null=no data, true=agrees, false=disagrees
let polyUp=null;
if(polymarketOdds.upPrice&&(Date.now()-polymarketOdds.lastFetch)<60000){
  polyUp=polymarketOdds.upPrice;
  const marketSaysUp=polyUp>0.52; // Need clear signal, not just 50.1¢
  const marketSaysDown=polyUp<0.48;
  if(marketSaysUp||marketSaysDown){
    polyAgrees=(marketSaysUp===priceAbovePtb);
  }
}
...
// ORDERBOOK IMBALANCE: positive = more $ buying UP shares
const obImbalance=orderbookData.imbalance;
const obFresh=(Date.now()-orderbookData.lastUpdate)<30000;
const obStrong=Math.abs(obImbalance)>0.15; // 15%+ imbalance is meaningful

// STREAK TRACKING: count consecutive same-direction results
// If last 2-3 candles all went same way, next one likely flips (your observation!)
let streakDir=null,streakLen=0;
if(predictionHistory.length>=2){
  const recent=predictionHistory.slice(-5);
  for(let i=recent.length-1;i>=0;i--){
    const wasUp=recent[i].result==='correct'?recent[i].prediction:!recent[i].prediction;
    if(streakDir===null){streakDir=wasUp;streakLen=1}
    else if(wasUp===streakDir)streakLen++;
    else break;
  }
}
const streakReversal=streakLen>=3; // 3+ in a row → expect flip

// RSI + volume spike can upgrade reversion conditions
const rsiExtreme=(rs&&(rs>75||rs<25));
const reversionBoosted=(rsiExtreme||volumeSpike);
const useReversionLoose=(priceDiff>25&&isSpike&&reversionBoosted);

// ═══ DECISION LOGIC (priority order) ═══

if((useReversion||useReversionLoose)&&polyAgrees===false){
  finalPrediction=!priceAbovePtb;
  strategy='REVERSION (spike+market)';
}else if((useReversion||useReversionLoose)&&polyAgrees===null){
  finalPrediction=!priceAbovePtb;
  strategy='REVERSION (spike)';
}else if(useReversion&&polyAgrees===true){
  finalPrediction=priceAbovePtb;
  strategy='MOMENTUM (spike+market agrees)';
}else if(obFresh&&obStrong&&polyUp&&Math.abs(polyUp-0.5)>0.04){
  // ORDERBOOK + POLYMARKET agree → very strong signal
  const obSaysUp=obImbalance>0;
  const polySaysUp=polyUp>0.5;
  if(obSaysUp===polySaysUp){
    finalPrediction=obSaysUp;
    strategy='ORDERBOOK+POLY (OB:'+Math.round(obImbalance*100)+'%, Poly:'+Math.round(polyUp*100)+'¢)';
  }else{
    // Disagree — trust Polymarket (more money at stake)
    finalPrediction=polySaysUp;
    strategy='POLYMARKET over OB ('+Math.round(polyUp*100)+'¢ vs OB:'+Math.round(obImbalance*100)+'%)';
  }
}else if(polyUp&&Math.abs(polyUp-0.5)>0.08){
  finalPrediction=polyUp>0.5;
  strategy='POLYMARKET (strong '+Math.round(polyUp*100)+'¢)';
}else if(priceDiff>15&&dominance>0.65){
  if(trendStrength>0.3&&trendUp!==priceAbovePtb){
    finalPrediction=trendUp;
    strategy='TREND REVERSAL';
  }else{
    finalPrediction=priceAbovePtb;
    strategy='MOMENTUM ($'+priceDiff.toFixed(0)+')';
  }
}else if(streakReversal){
  // 3+ candles same direction → bet the flip
  finalPrediction=!streakDir;
  strategy='STREAK FADE ('+streakLen+'x '+(streakDir?'up':'down')+')';
}else if(trendStrength>0.2&&totalTicks>=4){
  finalPrediction=trendUp;
  strategy='CHART TREND ('+upTicks+'↑ '+downTicks+'↓)';
}else if(obFresh&&obStrong){
  finalPrediction=obImbalance>0;
  strategy='ORDERBOOK ('+Math.round(obImbalance*100)+'% imbalance)';
}else{
  finalPrediction=priceAbovePtb;
  strategy='DEFAULT (price '+(priceAbovePtb?'above':'below')+')';
}

// CONFIDENCE
let confidence=50;
if(strategy.includes('REVERSION')&&polyAgrees===false){confidence+=20;if(rsiExtreme)confidence+=5;if(volumeSpike)confidence+=5}
else if(strategy.includes('ORDERBOOK+POLY'))confidence+=18+Math.abs(obImbalance)*20;
else if(strategy.includes('POLYMARKET'))confidence+=Math.abs(polyUp-0.5)*60;
else if(strategy.includes('STREAK'))confidence+=Math.min(streakLen*4,12);
else if(strategy.includes('MOMENTUM'))confidence+=Math.min(priceDiff/8,15)+Math.max(0,(dominance-0.5)*20);
else if(strategy.includes('ORDERBOOK'))confidence+=Math.abs(obImbalance)*25;
else confidence+=2;
const clampedConf=Math.min(confidence,92).toFixed(1);

// Build signal display
const signals=[];
signals.push({name:'Gap: $'+priceDiff.toFixed(0)+(priceAbovePtb?' above':' below'),dir:priceAbovePtb,weight:priceDiff/10});
if(ptbTracker.totalChecks>=3)signals.push({name:'Consistency '+Math.round(dominance*100)+'%',dir:aboveRatio>0.5,weight:dominance*5});
if(isSpike)signals.push({name:'⚡ Spike ('+Math.round(spikeRatio*100)+'%)',dir:!priceAbovePtb,weight:spikeRatio*5});
if(rs){const rsiDir=rs>50;signals.push({name:'RSI '+rs.toFixed(0)+(rs>75?' 🔴OB':rs<25?' 🟢OS':''),dir:rsiDir,weight:rsiExtreme?4:1})}
if(volumeSpike)signals.push({name:'📊 Vol '+volumeSpikeRatio.toFixed(1)+'x',dir:!priceAbovePtb,weight:3});
if(obFresh)signals.push({name:'📚 Orderbook '+(obImbalance>0?'+':'')+Math.round(obImbalance*100)+'%',dir:obImbalance>0,weight:Math.abs(obImbalance)*10});
if(polyUp)signals.push({name:'Polymarket '+Math.round(polyUp*100)+'¢',dir:polyUp>0.5,weight:Math.abs(polyUp-0.5)*20});
if(streakLen>=2)signals.push({name:'🔄 Streak: '+streakLen+'x '+(streakDir?'up':'down'),dir:!streakDir,weight:streakLen*2});
signals.push({name:'→ '+strategy,dir:finalPrediction,weight:0});

const predSource=strategy;
...
if(!predictionLock.locked){
  if(shouldLock){
    ...
    lockStatus=' 🔒 LOCKED';
    console.log('🔒 LOCKED: '+(finalPrediction?'YES ↑':'NO ↓')+' | '+strategy+' | Gap: $'+priceDiff.toFixed(2)+' | Spike: '+Math.round(spikeRatio*100)+'% | Poly: '+(polyUp?Math.round(polyUp*100)+'¢':'--'));
  }else{
    lockStatus='analyzing';
    const secsToLock=Math.ceil(lockAfter-timeInSlot);
    m.currentPrediction={willBeat:finalPrediction,confidence:clampedConf,priceToBeat:ptb,currentPrice:price,timestamp:Date.now(),targetTime:slotEnd,signals:pred.signals,features:pred.features,modelPredictions:pred.modelPredictions,pattern:pred.pattern};
    ...
    document.getElementById('predictionValue').innerHTML='<div style="font-size:1.1em;color:var(--tx2)">⏳ Analyzing... ('+secsToLock+'s)</div><div style="font-size:.52em;margin-top:4px;color:var(--tx3)">'+(priceAbovePtb?'↑ +$':'↓ -$')+priceDiff.toFixed(0)+' from PTB</div>';
    document.getElementById('predictionConfidence').textContent='Leaning: '+(finalPrediction?'YES ↑':'NO ↓');
...
m.currentPrediction={willBeat:displayWillBeat,confidence:displayConf,priceToBeat:ptb,currentPrice:price,timestamp:Date.now(),targetTime:slotEnd,signals:pred.signals,features:pred.features,modelPredictions:pred.modelPredictions,pattern:pred.pattern};
...
function updateSignalLearning(signals,wasCorrect,predictedUp){
  ...
  signals.forEach(sig=>{
    ...
    if(sigAgreed){
      ...
      if(wasCorrect){
        // Signal disagreed but prediction was right anyway — slight penalty
        sm.wrong++;
        sm.mult=Math.max(sm.mult*0.92,0.1);
      }else{
        // Signal disagreed and prediction was wrong — signal was right!
        sm.correct++;
...
[5,15,60].forEach(tf=>{
  ...
  // Don't set PTB on startup — wait for Chainlink boundary capture
